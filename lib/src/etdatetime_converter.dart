...
static const _mata = 'MATA';
static const _lelit = 'LELIT';
static const _tewat = 'TEWAT';
static const _qen = 'QEN';
   ...
   static DateTime toGregorian(ETDateTime etdatetime)
  {
    ...
    switch(etdatetime.month){
      ...
      if(etdatetime.timeofday==_lelit){
        hour = etdatetime.hour - 6;
        day++;
      }else if(etdatetime.timeofday==_tewat){
        hour = etdatetime.hour + (etdatetime.hour==12 ? - 6: 6);
      }else if(etdatetime.timeofday==_qen){
        hour = etdatetime.hour + 6;
      }else{
        hour = etdatetime.hour + (etdatetime.hour!=12 ? 18: 6);
      }
      //   else if(etdatetime.hour<6 && etdatetime.timeofday==_mata){
      //     hour = etdatetime.hour + 12;
      // }
      // else if(etdatetime.hour==12 && etdatetime.timeofday==_tewat){
      //   hour = etdatetime.hour - 6;
      // }
      // else{
      //   hour = etdatetime.hour + 6;
  ...
  static ETDateTime toEthiopian(DateTime datetime) {
    ...
    if(datetime.hour<6){
      ...
      timeofday = _tewat;
      hour = datetime.hour + (datetime.hour==6 ? 6 : -6);
    }
      ...
      timeofday = _mata;
      hour = datetime.hour + (datetime.hour==18 ? -6 : -18);
    }
    
    
    return ETDateTime(year,month,day,hour,datetime.minute,datetime.second,datetime.millisecond,datetime.microsecond,weekday,timeofday);
  }
